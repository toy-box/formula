...
export const MID = (...args: DataType[]) => {
  ...
  if (args.length == 3) {
    ...
    if (!args[1].isDecimalLike) {
      errors.push(
        new ArgumentErrorModel(ArugumentErrorCode.PARAM_TYPE, '必须为数字', 1),
      );
    }
    if (!args[2].isDecimalLike) {
      errors.push(
        new ArgumentErrorModel(ArugumentErrorCode.PARAM_TYPE, '必须为数字', 2),
      );
    }
  } else {
    errors.push(
...
export const SUBSTITUTE = (...args: DataType[]) => {
  ...
  if (args.length >= 3 && args.length <= 4) {
    ...
    if (args.length >= 4 && !args[3].isDecimalLike) {
      errors.push(
        new ArgumentErrorModel(ArugumentErrorCode.PARAM_TYPE, '必须为数字', 3),
      );
    }
  } else {
    errors.push(
